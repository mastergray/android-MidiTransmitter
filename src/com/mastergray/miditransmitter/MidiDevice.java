...
public class MidiDevice {
	...
	public static UsbDevice getOutputDevice(HashMap<String, UsbDevice> deviceList) {
				...
				for (int i = 0; i < devices.size(); i++) {
					...
					UsbInterface intf = setInterface(device);
						...
						if (intf != null) {
							...
							//	Check for MIDI In:
							UsbEndpoint epIn = setEndpointIn(intf);
	...
	public static UsbDevice getInputDevice(HashMap<String, UsbDevice> deviceList) {
				...
				for (int i = 0; i < devices.size(); i++) {
					...
					UsbInterface intf = setInterface(device);
						...
						if (intf != null) {
							...
							//	Check for MIDI Out:
							UsbEndpoint epIn = setEndpointOut(intf);
	...
	public static byte[] sendOutputRequest() {
	...
	public static byte[] sendInputRequest(byte[] midiData) {
		
		int transfer = connection.bulkTransfer(Midi_OUT, midiData, midiData.length, 0);
	...
	public static byte[] setMidiMessage(int command, int channel, int data1, int data2) {
		
		byte[] midiData = new byte[4];
		
		midiData[0] = (byte) command; // type of MIDI message;
		midiData[1] = (byte) channel; // which channel to send message to on the device;
		midiData[2] = (byte) data1;   // note value or command threshold
		midiData[3] = (byte) data2;   // note velocity or command parameter
